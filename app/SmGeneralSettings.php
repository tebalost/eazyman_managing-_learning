...
class SmGeneralSettings extends Model
{
    ...
    {
        try {
            $value = SmGeneralSettings::where('school_id', Auth::user()->school_id)->first();;
    ...
    public static function make_merit_list($InputClassId, $InputSectionId, $InputExamId)
    {
        try {
            ...
            $allresult_data = SmTemporaryMeritlist::where('iid', $iid)->orderBy('merit_order', 'asc')->where('school_id', Auth::user()->school_id)->get();
            $data['iid'] = $iid;
            $data['exams'] = $exams;
