            ...
            if ($user != "") {
                if (Hash::check($request->password, $user->password)) {
                    ...
                    if ($role_id == 2) {
                        ...
                        $data['sponsor'] = DB::table('sm_students')->select('sm_base_setups.base_setup_name as name')
                            ->join('sm_base_setups', 'sm_base_setups.id', '=', 'sm_students.sponsor_id')
                            ->where('sm_students.user_id', $user->id)
                            ->first();

                        $data['orphan'] = DB::table('sm_students')->select('sm_base_setups.base_setup_name as name')
                            ->join('sm_base_setups', 'sm_base_setups.id', '=', 'sm_students.orphan_id')
                            ->where('sm_students.user_id', $user->id)
                            ->first();
