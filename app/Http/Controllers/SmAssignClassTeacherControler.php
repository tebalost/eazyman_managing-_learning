...
class SmAssignClassTeacherControler extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $class_teachers = SmClassTeacher::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            $assigned_class_teachers=[];

            foreach($class_teachers as $class_teacher){
                $assigned_class_teachers[]=$class_teacher->teacher_id;
            }
            $school_class_teachers = $assigned_class_teachers;
            ...
            return view('backEnd.academics.assign_class_teacher', compact('classes', 'teachers', 'assign_class_teachers', 'school_class_teachers'));
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        ...
        $duplicate_class_teachers = SmAssignClassTeacher::where('active_status', 1)->where('class_id',$request->class)->where('section_id',$request->section)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->first();

        if($duplicate_class_teachers){
            Toastr::error('Duplicate class found', 'Failed');
            return redirect()->back();
        }
        if ($validator->fails()) {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
