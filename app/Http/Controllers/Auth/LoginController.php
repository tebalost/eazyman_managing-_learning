...
class LoginController extends Controller
{
    ...
    {
        $sbsUnAuthorisedUser = \Modules\RolePermission\Entities\User::where('email', $request->email)->first();
        if (!is_null($sbsUnAuthorisedUser))
            $request['school_id'] = $sbsUnAuthorisedUser->school_id ;
    ...
    public function login(Request $request)
    {
        ...
        if ($this->attemptLogin($request)) {
            ...
            $smGeneralSettings = SmGeneralSettings::where('school_id', Auth::user()->school_id)->first();

            // System date format save in session
            $date_format_id = $smGeneralSettings->date_format_id;
            ...
            // System academic session id in session
            $session_id = $smGeneralSettings->session_id;
            session()->put('sessionId', $session_id);

            session()->put('generalSetting',$smGeneralSettings);
            ...
            //Session put text decoration
            $ttl_rtl = $smGeneralSettings->ttl_rtl;
            session()->put('text_direction', $ttl_rtl);


//            $system_date_format = SmDateFormat::find($smGeneralSettings->date_format_id);
            session()->put('system_date_foramt', $system_date_format);

            $active_style = SmStyle::where('school_id', Auth::user()->school_id)->where('is_active', 1)->first() ;
            SmStyle::where('school_id', Auth::user()->school_id)->where('is_active', 1)->first();
            session()->put('active_style', $active_style);

            $all_styles = SmStyle::where('school_id', Auth::user()->school_id)->where('active_status', 1)->get() ;
            session()->put('all_styles', $all_styles);
