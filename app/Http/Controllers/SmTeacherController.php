...
class SmTeacherController extends Controller
{
    ...
    public function uploadContentEdit($id)
    {
        ...
        $editData = SmTeacherUploadContent::where('school_id', Auth::user()->school_id)->where('academic_id',getAcademicId())->where('id',$id)->first();
        // return $editData;
    ...
    public function studyMetarialList(Request $request)
    {
        ...
        try{
            if (Auth()->user()->role_id == 1) {
                ...
                $uploadContents = SmTeacherUploadContent::where('content_type', 'st')->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
    ...
    public function syllabusList(Request $request)
    {
        try{
            if (Auth()->user()->role_id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'sy')->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where(function ($q) {
                    $q->where('created_by', Auth::user()->id)->orWhere('available_for_admin', 1);
    ...
    public function otherDownloadList(Request $request)
    {
        ...
        try{
            if (Auth()->user()->role_id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'ot')->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where(function ($q) {
                    $q->where('created_by', Auth::user()->id)->orWhere('available_for_admin', 1);
    ...
    public function deleteUploadContent(Request $request, $id)
    {
        ...
        try{
            ...
            if (Auth::user()->role_id != 1 && $uploadContent->created_by == Auth::user()->id) {
