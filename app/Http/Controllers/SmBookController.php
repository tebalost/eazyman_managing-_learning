...
class SmBookController extends Controller
{
    ...
    public function issueBooks(Request $request, $member_type, $student_staff_id)
    {
        ...
        try {
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                $data['to talIssuedBooks'] = $totalIssuedBooks->toArray();
                return ApiBaseMethod::sendResponse($data, null);
            }
    ...
    public function saveIssueBookData(Request $request)
    {
        ...
        $check_issue_status = SmBookIssue::where('member_id', $request->member_id)
            ...
            ->where('book_number', $request->book_no)
        ...
        try {
            ...
            $bookIssue->book_number = $request->book_no;
    ...
    public function issueupdate(Request $request, $id, $member_type, $student_staff_id)
    {
        try{
            $memberDetails = SmLibraryMember::where('school_id', Auth::user()->school_id)->where('student_staff_id', '=', $student_staff_id)->first();
            ...
            if (checkAdmin()) {
                $issued_book = SmBookIssue::find($id);
            }else{
                $issued_book  = SmBookIssue::where('id',$id)
                    ->where('school_id',Auth::user()->school_id)
                    ->first();
            }
            $issued_books = SmBookIssue::where('academic_id', getAcademicId())
                ->where('school_id',Auth::user()->school_id)
                ->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
                $data['issued_book'] = $issued_book->toArray();
                $data['memberDetails'] = $memberDetails->toArray();
                $data['books'] = $books->toArray();
                $data['to talIssuedBooks'] = $totalIssuedBooks->toArray();
                return ApiBaseMethod::sendResponse($data, null);
            }
            return view('backEnd.library.issueBooks   ', compact('issued_book', 'issued_books','memberDetails', 'books', 'getMemberDetails', 'totalIssuedBooks'));
        }catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function destroy(Request $request, $id)
     {
         ...
         try{
                 ...
                 $book_issue = SmBookIssue::where('id',$id)->where('school_id',Auth::user()->school_id)->first();
                 $delete_book = SmBookIssue::where('id',$id)->where('school_id',Auth::user()->school_id)->delete();

                 if ($delete_book) {
                     $books = SmBook::find($book_issue->book_id);
                     $books->quantity = $books->quantity + 1;
                     $result = $books->update();
                     Toastr::success('Operation successful', 'Success');
                     return redirect()->back();
    ...
    public function allIssuedBook(Request $request)
    {
        try {
            ...
            $issueBooks = SmBookIssue::join('sm_books', 'sm_book_issues.book_id', '=', 'sm_books.id')
                ...
                ->join('sm_students', 'sm_students.user_id', '=', 'sm_library_members.student_staff_id')
                ->where('sm_books.school_id', Auth::user()->school_id)
                ->get();
