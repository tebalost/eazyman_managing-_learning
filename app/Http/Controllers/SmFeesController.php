...
class SmFeesController extends Controller
{
    ...
    public function feesGenerateModal(Request $request, $amount, $student_id, $type,$master)
    {
        ...
        try {
            ...
            $data['cash'] = SmPaymentGatewaySetting::where('gateway_name', 'Cash')->where('school_id', Auth::user()->school_id)->first();
            $data['eazymoola'] = SmPaymentGatewaySetting::where('gateway_name', 'Eazymoola')->where('school_id', Auth::user()->school_id)->first();
            $data['ecocash'] = SmPaymentGatewaySetting::where('gateway_name', 'Ecocash')->where('school_id', Auth::user()->school_id)->first();
            $data['sponsor'] = SmPaymentGatewaySetting::where('gateway_name', 'Sponsor')->where('school_id', Auth::user()->school_id)->first();
            $data['mpesa'] = SmPaymentGatewaySetting::where('gateway_name','like','%'.'Mpesa'.'%')->where('school_id', Auth::user()->school_id)->first();
            ...
            $method['cash'] = SmPaymentMethhod::where('method', 'Cash')->where('school_id', Auth::user()->school_id)->first();
            $method['eazymoola'] = SmPaymentMethhod::where('method', 'Eazymoola')->where('school_id', Auth::user()->school_id)->first();
            $method['ecocash'] = SmPaymentMethhod::where('method', 'Ecocash')->where('school_id', Auth::user()->school_id)->first();
            $method['sponsor'] = SmPaymentMethhod::where('method', 'Sponsor')->where('school_id', Auth::user()->school_id)->first();
            $method['mpesa'] = SmPaymentMethhod::where('method','like','%'.'Mpesa'.'%')->where('school_id', Auth::user()->school_id)->first();
            ...
            return view('backEnd.feesCollection.fees_generate_modal', compact('amount','master','banks','sponsors','discounts', 'fees_type_id', 'student_id', 'data', 'method'));
        } catch (\Exception $e) {
            dd($e);
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
...
/*        $request->validate([
        ...
        $response = Http::withHeaders([
            ...
            "accept" => ""
        ])->post("http://35.225.221.35:2022/api/pay-by-user-wallet/wd09pK", [$json]);
        ...
        try {
            ...
            // }
            if($request->payment_mode=="sponsor"){
                $mode = $request->sponsor_name;
            }
            elseif ($request->payment_mode=="bank"){
                $mode = $request->bank_name;
            }
            else{
            ...
            $fees_payment->payment_mode = $mode;
    ...
    public function feesPaymentDelete(Request $request)
    {
        ...
        try {
            $result = DB::table('sm_fees_payments')->where('id',$request->id)->where('school_id',Auth::user()->school_id)->delete();
    ...
    public function feesDueSearch(Request $request)
    {
        ...
        try {
            ...
            $fees_master = SmFeesMaster::select('id', 'amount')->where('fees_group_id', $fees_group[0])->where('fees_type_id', $fees_group[1])->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->first();
                if($request->section == ""){
                    $students = SmStudent::where('active_status', 1)->where('class_id', $request->class)->where('school_id',Auth::user()->school_id)->get();
                }else{
...
// return  $fees_assigneds;
            ...
            return view('backEnd.feesCollection.fees_payment_invoice_print')->with(['fees_assigneds' => $fees_assigneds, 'student' => $student,'unapplied_discount_amount'=>$unapplied_discount_amount, 'parent' => $parent, 'school'=>$school]);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    }
    public function feesPaymentQuotationPrint($id, $s_id)
    {
        try {
            set_time_limit(2700);
            $groups = explode("-", $id);
...
// return  $fees_assigneds;
            $unapplied_discount_amount = SmFeesAssignDiscount::where('student_id',$s_id)->sum('unapplied_amount');
            return view('backEnd.feesCollection.fees_payment_quotation_print')->with(['fees_assigneds' => $fees_assigneds, 'student' => $student,'unapplied_discount_amount'=>$unapplied_discount_amount, 'parent' => $parent, 'school'=>$school]);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    }
    public function feesPaymentReceiptPrint($id, $s_id)
    {
        try {
            set_time_limit(2700);
            $groups = explode("-", $id);
...
// return  $fees_assigneds;
            $unapplied_discount_amount = SmFeesAssignDiscount::where('student_id',$s_id)->sum('unapplied_amount');
            return view('backEnd.feesCollection.receipt_print')->with(['fees_assigneds' => $fees_assigneds, 'student' => $student,'unapplied_discount_amount'=>$unapplied_discount_amount, 'parent' => $parent, 'school'=>$school]);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
