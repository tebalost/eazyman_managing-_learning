...
class SmStudentAdmissionController extends Controller
{
    ...
    public function admission()
    {
        try {
            ...
            $academicYear = SmAcademicYear::where('active_status', '=', '1')->where('school_id', Auth::user()->school_id)->first()->year;
            $school_code = SmGeneralSettings::where('school_id', Auth::user()->school_id)->first()->school_code;

            $next_student = $max_admission_id + 1;
            if (strlen($next_student) == 1) {
                $next_student = "000" . $next_student;
            } else if (strlen($next_student) == 2) {
                $next_student = "00" . $next_student;
            } else if (strlen($next_student) == 3) {
                $next_student = "0" . $next_student;
            }
            $admission_id_number = $school_code . $academicYear . $next_student;

            $classes = SmClass::where('active_status', '=', '1')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();

            $religions = SmBaseSetup::where('active_status', '=', '1')->where('base_group_id', '=', '2')->get();
            $sponsors = SmBaseSetup::where('active_status', '=', '1')->where('base_group_id', '=', '4')->get();
            $orphans = SmBaseSetup::where('active_status', '=', '1')->where('base_group_id', '=', '5')->get();
            ...
            return view('backEnd.studentInformation.student_admission', compact('classes', 'religions', 'blood_groups', 'genders', 'route_lists', 'vehicles', 'dormitory_lists', 'categories', 'groups', 'sessions', 'max_admission_id', 'admission_id_number', 'max_roll_id', 'driver_lists', 'payment_methods', 'reference', 'sponsors', 'orphans'));
        } catch (\Exception $e) {
...
/*        if ($request->email_address != "") {
        ...
        //Get the Max Roll and Admission Numbers
        $max_admission_id = SmStudent::where('school_id', Auth::user()->school_id)->max('admission_no');
        $max_roll_id = SmStudent::where('school_id', Auth::user()->school_id)->max('roll_no');
        $academicYear = SmAcademicYear::where('active_status', '=', '1')->where('school_id', Auth::user()->school_id)->first()->year;
        $school_code = SmGeneralSettings::where('school_id', Auth::user()->school_id)->first()->school_code;
        ...
        try {
            ...
            $user_stu->username = $admission_id_number;
            if($request->email_address != "") {
                $user_stu->email = $request->email_address;
            }else{
                $user_stu->email = $admission_id_number;
            }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        $student->admission_no = $next_student;
                        ...
                        $student->admission_id_number = $admission_id_number;
                        ...
                        if (@$request->orphan != "") {
                            $student->orphan_id = $request->orphan;
                        }
    ...
    public function studentView(Request $request, $id)
    {
         try {
            ...
            //Subjects
             $assigned_subjects = DB::table('sm_assign_subjects')
                 ...
                 ->select('sm_subjects.subject_name','sm_subjects.id', 'sm_subjects.subject_code', 'sm_subjects.subject_type', 'sm_staffs.full_name as teacher_name')
                 ->where('sm_assign_subjects.class_id', '=', $student_detail->class_id)
                 ->where('sm_assign_subjects.section_id', '=', $student_detail->section_id)
                 ->where('sm_assign_subjects.academic_id', getAcademicId())->where('sm_assign_subjects.school_id',Auth::user()->school_id)->get();

             //Optional Subjects              sm_optional_subject_assigns

             $assigned_optional_subjects = DB::table('sm_optional_subject_assigns')
                 ->leftjoin('sm_subjects', 'sm_subjects.id', '=', 'sm_optional_subject_assigns.subject_id')
                 ->select('sm_subjects.subject_name', 'sm_subjects.id', 'sm_subjects.subject_code', 'sm_subjects.subject_type')
                 ->where('sm_optional_subject_assigns.student_id', '=', $student_detail->id)
                 ->where('sm_optional_subject_assigns.academic_id', getAcademicId())->where('sm_optional_subject_assigns.school_id',Auth::user()->school_id)->get();

             //Optional Subjects for the class

             $optional_subjects = DB::table('sm_optional_subject_assigns')
                ->where('sm_optional_subject_assigns.school_id',Auth::user()->school_id)
                 ->where('sm_optional_subject_assigns.class_id', '=', $student_detail->class_id)
                 ->where('sm_optional_subject_assigns.section_id', '=', $student_detail->section_id)
            ...
            // getting wallet balance
             ...
             $guardian_mobile = $student_detail->parents->guardians_mobile;
             $name = $student_detail->full_name;
             $userWalletInfo = [];

             if(!empty($mobile) || !empty($guardian_mobile)) {
                 if(!isset($mobile) && isset($guardian_mobile)){
                     $mobile = $student_detail->parents->guardians_mobile;
                     $name = $student_detail->parents->guardians_name;
                 }
                 try {
                     $response = Http::get('http://35.225.221.35:2025/api/get-user-wallet-account/msisdn/' . $mobile . '/user-name/' . $name . '');
                     $walletUserData = json_decode(json_encode($response->json()), false);
                     if(!isset($walletUserData->status)) {
                         $userWalletInfo = $walletUserData->walletAccountDto;

                         $eazymoola_update = SmStudent::find($student_detail->id);
                         $eazymoola_update->eazymoola_ref_no = $userWalletInfo->userWalletReferenceNumber;
                         $eazymoola_update->save();
                     }
                 } catch (Exception $e) {
                     Toastr::error('Operation Failed', 'Failed');
                     return redirect()->back();
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                $data['assigned_subjects'] = $assigned_subjects->toArray();
                $data['documents'] = $documents->toArray();
                $data['timelines'] = $timelines->toArray();
                $data['siblings'] = $siblings->toArray();
                $data['optional_subjects'] = $optional_subjects->toArray();
                $data['assigned_optional_subjects'] = $assigned_optional_subjects->toArray();
            ...
            //Todo make call to eazymoola endpoint to get total


            return view('backEnd.studentInformation.student_view', compact('student_detail', 'driver_info', 'fees_assigneds', 'fees_discounts', 'exams', 'documents', 'timelines', 'siblings', 'grades', 'academic_year', 'exam_terms', 'payment_methods', 'userWalletInfo', 'assigned_subjects', 'assigned_optional_subjects', 'optional_subjects'));
        } catch (\Exception $e) {
             dd($e);
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    public function studentEdit(Request $request, $id)
    {
        try {
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                $data['sponsors'] = $sponsors->toArray();
                $data['orphans'] = $orphans->toArray();
            ...
            return view('backEnd.studentInformation.student_edit', compact('student', 'classes', 'religions', 'blood_groups', 'genders', 'route_lists', 'vehicles', 'dormitory_lists', 'categories', 'groups', 'sessions', 'siblings', 'driver_lists', 'sponsors', 'orphans'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
...
/*        $is_duplicate = SmParent::where('school_id', Auth::user()->school_id)->where('guardians_email', $request->guardians_email)->where('id', '!=', $student_detail->parent_id)->first();
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        $student->admission_id_number = $request->admission_id_number;
    ...
    public function importStudent()
    {
        try {
            ...
            $sponsors = SmBaseSetup::where('base_group_id', 4)->get();
            $orphans = SmBaseSetup::where('base_group_id', 5)->get();
            $sessions = SmAcademicYear::where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.studentInformation.import_student', compact('classes', 'genders', 'blood_groups', 'religions', 'sessions', 'sponsors', 'orphans'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    try {
                                        ...
                                        $student->sponsor_id = $value->sponsor;
                                        $student->orphan_id = $value->orphan;
