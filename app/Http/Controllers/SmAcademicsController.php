...
class SmAcademicsController extends Controller
{
    ...
    public function classReportPrint($class_id, $section_id)
    {
         try {
            $search_class = SmClass::where('academic_id', getAcademicId())->where('id', $class_id)->where('school_id', Auth::user()->school_id)->first();

            if ($section_id != "") {
                $section = SmSection::where('academic_id', getAcademicId())->where('id', $section_id)->where('school_id', Auth::user()->school_id)->first();
            } else {
                $section = '';
            }
            ...
            $assign_subjects->where('academic_id', getAcademicId())->where('active_status', 1);
            if ($section_id != "") {
                $assign_subjects->where('section_id', $section_id);
            }
            $assign_subjects->where('class_id', $class_id);
            $assign_subjects = $assign_subjects->where('school_id', Auth::user()->school_id)->get();
            ...
            return view('backEnd.reports.class_report_print', compact('classes', 'students','applied_discount', 'assign_subjects', 'assign_class_teachers', 'total_collection', 'total_assign', 'search_class', 'section'));
        } catch (\Exception $e) {
            // dd($e);
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
