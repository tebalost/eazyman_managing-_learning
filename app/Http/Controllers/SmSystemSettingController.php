...
class SmSystemSettingController extends Controller
{
    ...
    public function getEazyMoolaHost(){
        $hostname = env("EAZYMOOLA_URL", "http://35.225.221.35:2025/api/");
        return $hostname;
    }
    public function generalSettingsView(Request $request)
    {
        try {
            ...
            $response = Http::get($this->getEazyMoolaHost().'get-user-wallet-account/msisdn/' . $editData->school_code . '/user-name/' .$editData->school_name . '');
            ...
            $ref_no = $userWalletInfo->userWalletReferenceNumber;
           
            $session = SmGeneralSettings::join('sm_academic_years', 'sm_academic_years.id', '=', 'sm_general_settings.session_id')->find(Auth::user()->school_id);
            // return $editData;
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {

                return ApiBaseMethod::sendResponse($editData, null);
            }
            return view('backEnd.systemSettings.generalSettingsView', compact('editData', 'session','ref_no','balance'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function updateGeneralSettingsData(Request $request)
    {
        ...
        try {
            ...
            $generalSettData->motto           = $request->motto;
            $generalSettData->registration_no = $request->registration_no;
            ...
            $generalSettData->bank_name       = $request->bank_name;
            $generalSettData->bank_account    = $request->bank_account;
            $generalSettData->branch_code     = $request->bank_branch_code;
            $generalSettData->eazymoola_account = $request->eazymoola_account;
    ...
    public function updateSchoolOpeningDate(Request $request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
        ...
        try {
                ...
                $date               = date('Y-m-d', strtotime($request->date_of_opening)); ;
                $generalSettData          = SmGeneralSettings::where('school_id',Auth::user()->school_id)->first();
                $generalSettData->re_open_date = $date;
                $results                  = $generalSettData->update();
