...
<body>
@foreach($student_results as $key => $student_detail)
@php
$student_id = $student_detail['student_id'];
$report_status = DB::table('sm_temporary_meritlists')->where([['class_id', $class_id], ['section_id', $section_id], ['exam_id', $exam_type_id], ['student_id', $student_detail['student_id']]])
->where('academic_id',getAcademicId())->where('report_status',1)->first();
@endphp
...
<div class="student_marks_table">
            ...
            <td border="0" style="border: #ffffff">
                <div class="school_name">
                    ...
                    <p class="text_center" style="font-size: large"><strong> PERFORMANCE REPORT FOR </strong>
                        {{$exam_details->title}} ({{@$student_detail['academic_year']}})</p>
                </div>
            </td>
        </tr>
        </thead>
    ...
    @if($student_detail['overall_marks']!==0)
    @php
    $average_mark = $student_detail['overall_average'];
    $sum_of_mark = $student_detail['overall_marks'];
    @endphp
    @else
    @php
    $average_mark = $student_detail['average_mark'];
    $sum_of_mark = $student_detail['sum_of_mark'];
    @endphp
    @endif
        ...
        <tr>
            ...
            <th style="border: #ffffff" colspan="1" class="text_left">
                <ul class="info_details" style="margin-top: 0; margin-bottom: 0;">
                    ...
                    <li><p>Position in Class: &nbsp; &nbsp; </p> &nbsp; <p><strong>
                        {{@$student_detail['position']}} </strong> out of <strong>{{$students->count()}} Students</strong></p>
                    </li>
                    @endif
                    ...
                    <li><p>Position in Stream: &nbsp; &nbsp; </p> &nbsp; <p><strong>
                                {{@$student_detail['stream_position']}}</strong> out of <strong>{{@$student_detail['students_in_stream']}} Students</strong> in <strong>{{$class_name->class_name}}</strong></p>
                    </li>
                    @endif
                    <li><p>Subjects Passed: &nbsp; &nbsp; </p> &nbsp;
                        @if($student_detail['passed_subjects'] == 0)
                        <p><strong>{{$student_detail['number_of_passed_subjects']}}</strong></p>
                        @else
                        <p><strong>{{$student_detail['passed_subjects']}}</strong></p>
                        @endif
                    </li>
                    </li>
                </ul>
    ...
    <table class="custom_table">
        ...
        <tr>
            ...
            @if(count($student_detail['exam_course_work'])>0)
        ...
        @foreach($student_detail['course_work_mark'] as $marks => $data)
        @php
        ...
        $report_status = DB::table('sm_temporary_meritlists')->where([['class_id', $class_id], ['section_id', $section_id], ['exam_id', $exam_type_id], ['student_id', $student_id]])
        ...
        @if($student_detail['final_result']!=="")
        <tr style="border: #0b0b0b 2px solid">
            <td colspan="4" style="text-align: left;border: #0b0b0b 2px solid"><strong>Result</strong></td>
            <td colspan="3" style="text-align: center;border: #0b0b0b 2px solid"><strong>{{$student_detail['final_result']}}</strong></td>
            <td colspan="2"></td>
        </tr>
        @endif
        </tbody>
    ...
    <table class="custom_table" border="0">
        ...
        @if(in_array($class_id,$stream))
        <tr>
            <td @if($result_settings->result_name==$student_detail['final_result']) style="background: #ddd" @endif>{{$result_settings->result_name}}</td>
            <td @if($student_detail['average_mark']>=$result_settings->percent_from && $student_detail['average_mark']<=$result_settings->percent_upto) style="background: #ddd" @endif>{{$result_settings->percent_from}}-{{$result_settings->percent_upto}}%</td>
    ...
    <table class="custom_table border-0">
        ...
        <tr>
            ...
            <td class="border-0" style="width: 60%">
                <p style="font-size:14px; border-bottom:1px solid gray; text-align: left">
                    <em>{{$class_teacher_remark}}</em></p>
        ...
        <tr>
            ...
            <td class="border-0">
                <p style="font-size:14px; border-bottom:1px solid gray; text-align: left; font-style: italic">
                    {{$principal_remark}}
                </p>
            </td>
            <td class="border-0">
                ...
                @if(in_array($class_name->class_name,$primary))
                @php
                $principal_details = DB::table('sm_staffs')
                ->whereRaw('department_id IN (SELECT id from sm_human_departments WHERE name=\'PRIMARY\')')
                ->whereRaw('designation_id IN (SELECT id FROM sm_designations WHERE title=\'Principal\')')
                ->first();

                @endphp
                <p style="text-align:center; border-top:1px solid #0b0b0b; display:inline-block; margin-top:10px;"><strong><em>{{$principal_details->full_name}}</em></strong><br>
                @else
