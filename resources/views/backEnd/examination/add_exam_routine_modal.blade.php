    ...
    {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'route' => 'add-exam-routine-store',
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="row no-gutters input-right-icon mt-35">
                    <div class="col">
                        <div class="input-effect">
                            <input class="primary-input date" id="startDate" type="text" name="date" value="{{isset($assigned_exam)? date('m/d/Y', strtotime($assigned_exam->date)):''}}" readonly="true">
                ...
                <div class="row mt-25">
                    <div class="col-lg-12 mt-30-md">
                        <select class="w-100 bb niceSelect form-control" name="invigilator" id="invigilator">
                            <option data-display="Select invigilator " value="">@lang('lang.select') @lang('lang.invigilator')</option>
                            @foreach($invigilators as $invigilator)
                            <option value="{{ @$invigilator->id}}" {{isset($assigned_exam)? ($assigned_exam->teacher_id == $invigilator->id? 'selected':''):''}}>{{ @$invigilator->full_name}}</option>
                            @endforeach
                        </select>
                        <span class="text-danger" role="alert" id="invigilator_error">
                        </span>
                    </div>
                </div>
            </div>
