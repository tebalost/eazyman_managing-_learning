...
class Webinar extends Request
{
    ...
    {
    	return $this->patch("webinars/{$webinarId}", ['status' => $status]);
    }
    ...
    {
    	$response = $this->get("webinars/{$webinarId}/panelists");
    	$panelists = [];
     ...
     * @return array|mixed
     */
    public function listRegistrants($webinarId)
    {
    ...
    public function updateRegistrantStatus($webinarId, $registrants, $action)
    {
    	...
    	return $this->patch("webinars/{$webinarId}/registrants/status", ['action' => $action, 'registrants' => $reges]);
    }
